...
async def test_on_chat_start_initializes_state_and_sends_welcome(mock_cl): 
    ...
    with patch.object(app, 'cl', new=mock_cl) as mock_cl_in_app:
        ...
        # Check that cl.user_session.set was called for insight_flow_state
        # and other UI settings
        # Expected calls: insight_flow_state, direct_mode, show_perspectives, show_visualization, quick_mode,
        # persona_factory, progress_msg (though progress_msg might be None initially)
        # The original test had 7. If we add UI elements that set session state, this might change.
        # For now, let's assume it remains 7 from the original setup and new UI elements are passive until interacted with, 
        # or their initial state is part of the main insight_flow_state.
        # The persona selection action itself doesn't set state, it just creates an action.
        # The Display/Mode switches will add to user_session.set calls in on_chat_start.

        # --- Previous assertions for state and welcome message ---
        ...
        for call_args_obj in mock_cl_in_app.Message.call_args_list:
            ...
            if content == "Welcome to InsightFlow AI!":
            ...
            if content == "Configure your research team:":
                ...
                if actions and isinstance(actions, list) and len(actions) == 1:
                    # This action_in_message is mock_cl_in_app.Action.return_value
                    action_in_message = actions[0] 

        assert found_welcome_message, "Welcome message not found"
        assert found_persona_action_message_text, "Message text 'Configure your research team:' not found"
        assert action_in_message is not None, "No action found in the 'Configure your research team:' message"
        
        # Assert that cl.Action (the class mock) was called correctly
        ...
        # mock_cl_in_app.Message.return_value.send should have been called at least twice
        assert mock_cl_in_app.Message.return_value.send.call_count >= 2
